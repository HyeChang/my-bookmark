...
const bookmarkCardCss = readFileSync(
  resolve(__dirname, "../components/BookmarkCard.css"),
  "utf8"
);
...
const homePanelSource = readFileSync(
  resolve(__dirname, "../components/HomePanel.tsx"),
  "utf8"
);
...
describe("bookmark mobile overflow css", () => {
  it("keeps long bookmark text from widening mobile bookmark rows", () => {
    ...
    expect(bookmarkCardCss).toMatch(
  ...
  it("lets home favorite action menus use the same adaptive mobile placement", () => {
    expect(homePanelSource).toContain("useActionMenuPlacement");
    expect(homePanelSource).toContain("data-open-action-menu");
    expect(homePanelSource).toContain("data-menu-placement={actionMenuPlacement}");
  });
