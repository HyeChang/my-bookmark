...
- 최신 기능 커밋:
  - `bd48684` `feat: compact desktop search panel`
...
- 데스크톱 작업 공간 정리:
  ...
  - 중앙 `결과 컬럼`
  - 우측 `읽기 레일`
- 읽기 레일에 상세 선택 전 안내 플레이스홀더 표시
- 데스크톱 검색 패널은 압축형 툴바 구조
- 모바일 검색 패널 접힘 / 펼침 지원
- 모바일 작업 패널 탭형 전환 완료:
  - 북마크
  - 폴더
...
- 좌측 `작업 패널`, 중앙 `결과 컬럼`, 우측 `읽기 레일` 위계 확인
- 작업 패널 3개가 공통 헤더 체계 사용
- 데스크톱 검색 패널이 압축형 툴바로 보이는지 확인
- 검색 패널에 `현재 작업 조건` 노출
- 추천 영역에 `빠른 진입점` 노출
- 상세 패널에 `읽기 중심`과 액션 그룹 라벨 노출
- 상세를 선택하지 않았을 때 안내 플레이스홀더 노출
